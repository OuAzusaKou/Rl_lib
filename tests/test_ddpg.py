...
from jueru.user.custom_actor_critic import MLPfeature_extractor, ddpg_actor, ddpg_critic, CNNfeature_extractor
from jueru.utils import Channel_First_wrapper

@pytest.mark.parametrize("feature_extractor", [CNNfeature_extractor])
def test_ddpg(feature_extractor):
    if feature_extractor == MLPfeature_extractor:
        env = gym.make('Pendulum-v1')

        feature_extractor = feature_extractor(env.observation_space, [], 3)
        ...
        data_collection_dict['replay_buffer'] = Replay_buffer(env=env, size=1e4)
    ...
    elif feature_extractor == CNNfeature_extractor:
        env = gym.make('CarRacing-v0')

        env = Channel_First_wrapper(env)

        feature_extractor = feature_extractor(env.observation_space)

        actor = ddpg_actor(env.action_space, feature_extractor)

        critic = ddpg_critic(env.action_space, feature_extractor)
