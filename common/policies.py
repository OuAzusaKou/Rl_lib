...
class BasePolicy(BaseModel):
    ...
    def predict(
        ...
        if isinstance(self.action_space, gym.spaces.Tuple):
            # print(actions[0][0])
            cl = np.array([actions[0][0]])
            # print(cl)
            conti = np.clip(actions[0][1:3],
                            self.action_space[1].low, self.action_space[1].high)
            # print(conti)
            clipped_actions = [np.concatenate((cl, conti))]
...
class ActorCriticPolicy(BasePolicy):
        ...
        if self.ortho_init:
            ...
            for module, gain in module_gains.items():
                if type(module) == tuple:
                    module[0].apply(partial(self.init_weights, gain=gain))
                    module[1].apply(partial(self.init_weights, gain=gain))
                else:
    ...
    def _get_action_dist_from_latent(self, latent_pi: th.Tensor,
        ...
        """
        if type(self.action_net) == tuple:
            mean_actions_dis = self.action_net[0](latent_pi)
            mean_actions_cont = self.action_net[1](latent_pi)
        else:
        ...
        if isinstance(self.action_dist, TupleDistribution):
            return self.action_dist.proba_distribution(mean_actions_dis, mean_actions_cont, self.log_std)
        else:
