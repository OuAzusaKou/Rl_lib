...
class Uav_env(gym.Env):
    ...
    def reset(self):
        ...
        # Initialize trash and agent location
        self.init_state()
        obs = self._get_obs()
        self.render_rest()
        # if self.render_mode:
    ...
    def excute_action(self, action):
        ...
        #print(np.array([np.cos(action * np.pi), np.sin(action * np.pi)]))
        self.agent_pos = self.agent_pos + self.step_size * np.array([np.cos(action * np.pi), np.sin(action * np.pi)])[0]
        # change robot pos if robot collide with the bound
        if self.agent_pos[0] > (self.world_size - 1) or self.agent_pos[1] > (self.world_size - 1) or self.agent_pos[0] < 0 or (
                self.agent_pos[1] < 0):
    ...
    def target_move(self):
        ...
        if self.target_list.sprites()[0].rect.x > (self.world_size - 1) or self.target_list.sprites()[0].rect.y > (self.world_size - 1) or self.target_list.sprites()[0].rect.x < 0 or (
                self.target_list.sprites()[0].rect.y < 0):
            self.target_list.sprites()[0].rect.x = np.clip(self.target_list.sprites()[0].rect.x, 0, (self.world_size - 1))
            self.target_list.sprites()[0].rect.y = np.clip(self.target_list.sprites()[0].rect.y, 0, (self.world_size - 1))
        return
